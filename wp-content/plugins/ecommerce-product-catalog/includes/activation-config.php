...
class ic_cat_activation_wizard extends ic_activation_wizard {
	...
	function activation_notices() {
		...
		if ( is_ic_activation_notice() && !$this->get_notice_status( 'notice-ic-catalog-activation' ) ) {
		...
		} else if ( is_ic_edit_product_screen() && !$this->get_notice_status( 'notice-ic-catalog-single-customize' ) ) {
			...
			if ( !empty( $product_id ) && is_ic_product( $product_id ) && ic_product_exists( $product_id ) ) {
				...
				$this->wizard_box( 'notice-ic-catalog-single-customize' );
			}
		} else if ( is_ic_product_list_admin_screen() && !$this->get_notice_status( 'notice-ic-catalog-listing-customize' ) ) {
			$display_notice	 = true;
			...
			$this->box_choice( $questions );
			$this->wizard_box( 'notice-ic-catalog-listing-customize' );
		} else if ( is_ic_catalog_admin_page() && !$this->get_notice_status( 'notice-ic-catalog-welcome' ) ) {
			$display_notice = true;
			...
			$this->wizard_box( 'notice-ic-catalog-welcome' );
		}
		if ( $display_notice ) {
